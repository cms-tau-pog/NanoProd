...
store_failed=$(python3 -c '
import PSet
print(PSet.process.exParams.storeFailed.value())
')
...
run_cmd python3 $CMSSW_BASE/python/NanoProd/NanoProd/run_skim.py $cmsDriver_out $final_out $store_failed
